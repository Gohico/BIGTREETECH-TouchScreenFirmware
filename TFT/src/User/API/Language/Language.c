...
u8 *textSelect(u8 sel)
{
  ...
  case CHINESE:
    return (u8 *)cn_pack[sel];
  case RUSSIAN:
    return (u8 *)ru_pack[sel];
  case JAPANESE:
    return (u8 *)jp_pack[sel];
  case ARMENIAN:
    return (u8 *)am_pack[sel];
  case GERMAN:
    return (u8 *)de_pack[sel];
  case CZECH:
    return (u8 *)cz_pack[sel];
  case SPAIN:
    return (u8 *)es_pack[sel];
  case FRENCH:
    return (u8 *)fr_pack[sel];
  case PORTUGUESE:
    return (u8 *)pt_pack[sel];
  case ITALIAN:
    return (u8 *)it_pack[sel];
  case POLISH:
    return (u8 *)pl_pack[sel];
  case SLOVAK:
    return (u8 *)sk_pack[sel];
  case DUTCH:
    return (u8 *)du_pack[sel];
  case HUNGARIAN:
    return (u8 *)hu_pack[sel];
